...
class SubscriptionForm extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            ...this.getDefaultState(props.subscription),
            isSubmitting: false
        };
    }
    ...
    getDefaultState(subscription = null) {
        let state = {
            values: {},
            errors: {},
            formCorrect: false,
        ...
        if(subscription) {
            fields.forEach(field => {
                state.values[field.name] = subscription[field.name];
                state.errors[field.name] = "";
            });
            state.values._id = subscription._id;
        } else {
            fields.forEach(field => {
    ...
    submitForm() {
        ...
        const action = this.props.subscription ? SubscriptionService.editSubscription : SubscriptionService.postNewSubscription;
    ...
    render() {
        ...
        const actionText = subscription ? "Editar" : "Añadir";
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(withStyles(styles, { withTheme: true })(SubscriptionForm)));
