...
it("clears form", () => {
  ...
  screen
    ...
    .forEach((field, i) => expect(field).toHaveValue(""));
});

it("add profile", () => {
  renderComponent();

  BillingProfileService.postNewProfile = jest
    .fn()
...
it("edit profile", () => {
  ...
  BillingProfileService.editProfile = jest
    .fn()
    .mockImplementation((profile) => {
      return new Promise((resolve, reject) => {
        resolve();
  ...
  expect(BillingProfileService.editProfile).toHaveBeenCalledWith({
    ...profile,
    name: newName,
  });
});
