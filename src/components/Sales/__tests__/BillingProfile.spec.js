...
const renderComponent = () =>
  render(
    <BillingProfile.WrappedComponent
      history={history}
      profile={profile}
...
it("should have profile fields", () => {
  ...
  expect(
    screen.queryByText("[zipCode] city, province, country")
  ).not.toBeNull();
...
it("fire delete button", async () => {
  ...
  BillingProfileService.deleteProfile = jest
    .fn()
  ...
  await waitFor(() =>
    expect(BillingProfileService.requestProfiles).toHaveBeenCalledTimes(1)
  );
});
